...
function ArenaCommandCenter() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return leaderboardItems.filter((item) => {
      // Name-only search: the API no longer returns emails or per-row colleges.
  ...
  }, [leaderboardItems, search, statusFilter]);

  const exportCSV = () => {
    const escapeCSV = (value: string) => `"${String(value).replace(/"/g, '""')}"`;
    // PII-minimal export: no email column (the API doesn't return one).
    const headers = ["Rank", "Name", "College", "Completed Challenges", "Total Score", "Status"];
    const rows = filteredItems.map((item, idx) => [
      idx + 1,
      escapeCSV(item.name),
      escapeCSV(leaderboard?.college_name || ""),
      `${item.completed}/5`,
      item.score,
          ...
          <div className="relative overflow-hidden rounded-2xl border border-amber-500/40 bg-gradient-to-br from-amber-500/10 via-card to-emerald-500/10 p-6 shadow-2xl">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between relative z-10">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {leaderboard?.college_name || ""} • {winner.completed} challenges completed • Finished in {winner.time_taken || "--:--"}
                </p>
              </div>
          ...
          <aside className="space-y-6">
              ...
              <ul className="space-y-3 text-xs">
                {leaderboardItems.length > 0 ? (
                  leaderboardItems.slice(0, 5).map((item, i) => (
                    <li key={item.rank ?? i} className="flex gap-2.5 border-b border-border/40 pb-2 last:border-0">
            ...
            <div className="rounded-2xl border border-border/80 bg-card p-5 space-y-4 shadow-xl">
              ...
              <ul className="space-y-2.5 text-xs">
                {(() => {
                  // One event per board — the college comes from the payload
                  // header, not per-row fields (rows are PII-minimal now).
                  if (totalParticipants === 0) {
                    return <li className="text-muted-foreground text-center py-4">No college data available</li>;
                  }
                  return (
                    <li className="flex items-center justify-between rounded-xl border border-border/50 bg-[var(--surface)] p-2.5">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-primary">#1</span>
                        <span className="font-bold text-foreground">{leaderboard?.college_name || "Unknown"}</span>
                        <span className="text-[10px] text-muted-foreground">({totalParticipants} participants)</span>
                      </div>
                      <span className="font-mono font-semibold text-emerald-400">{avgScore} Pts Avg</span>
                    </li>
                  );
                })()}
              </ul>
          ...
          <div className="relative w-full max-w-xl rounded-2xl border border-border/80 bg-card p-6 shadow-2xl space-y-5 backdrop-blur-xl animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-border/60 pb-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Rank #{selectedStudent.rank} • {leaderboard?.college_name || ""}</p>
              </div>
